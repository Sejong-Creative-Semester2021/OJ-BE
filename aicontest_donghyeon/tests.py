...
from .models import AIContestTag, AIProblemIOMode
from .models import AIContest, AIProblemRuleType
...
DEFAULT_PROBLEM_DATA = {"_id": "A-110", "title": "test", "description": "<p>test</p>", "summary_description": "test",
                        "rule_description": "test", "schedule_description": "test", "strat_time":"test", "end_time":"test", "memory_limit": 256, "difficulty": "Low",
...
class ProblemTagListAPITest(APITestCase):
    def test_get_tag_list(self):
        AIContestTag.objects.create(name="name1")
        AIContestTag.objects.create(name="name2")
...
class AddProblemFromPublicProblemAPITest(ProblemCreateTestBase):
    ...
    def test_add_contest_problem(self):
        resp = self.client.post(self.url, data=self.data)
        self.assertSuccess(resp)
        self.assertTrue(AIContest.objects.all().exists())
        self.assertTrue(AIContest.objects.filter(contest_id=self.contest["id"]).exists())
