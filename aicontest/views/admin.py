...
from ..models import AIContest, AIProblemRuleType, AIContestTag
...
class TestCaseAPI(CSRFExemptAPIView, TestCaseZipProcessor):
    ...
    def get(self, request):
        ...
        try:
            problem = AIContest.objects.get(id=problem_id)
        except AIContest.DoesNotExist:
...
class ProblemAPI(ProblemBase):
    ...
    def post(self, request):
        ...
        if AIContest.objects.filter(_id=_id, contest_id__isnull=True).exists():
        ...
        problem = AIContest.objects.create(**data)

        for item in tags:
            try:
                tag = AIContestTag.objects.get(name=item)
            except AIContestTag.DoesNotExist:
                tag = AIContestTag.objects.create(name=item)
    ...
    def get(self, request):
        ...
        problems = AIContest.objects.filter(contest_id__isnull=True).order_by("-create_time")
        if rule_type:
    ...
    def put(self, request):
        ...
        if not _id:
            return self.error("Display ID is required")
        if AIContest.objects.exclude(id=problem_id).filter(_id=_id, contest_id__isnull=True).exists():
            return self.error("Display ID already exists")
        ...
        for tag in tags:
            try:
                tag = AIContestTag.objects.get(name=tag)
            except AIContestTag.DoesNotExist:
                tag = AIContestTag.objects.create(name=tag)
            problem.tags.add(tag)
    ...
    def delete(self, request):
        ...
        try:
            problem = AIContest.objects.get(id=id, contest_id__isnull=True)
        except AIContest.DoesNotExist:
            return self.error("Problem does not exists")
        ensure_created_by(problem, request.user)
...
class ContestProblemAPI(ProblemBase):
    ...
    def post(self, request):
        ...
        if AIContest.objects.filter(_id=_id, contest=contest).exists():
    ...
    def get(self, request):
        ...
        try:
            ...
            return self.error("Contest does not exist")
        problems = AIContest.objects.filter(contest=contest).order_by("-create_time")
        if user.is_admin():
    ...
    def put(self, request):
        ...
        try:
            problem = AIContest.objects.get(id=problem_id, contest=contest)
        except AIContest.DoesNotExist:
            return self.error("Problem does not exist")
        ...
        if not _id:
            return self.error("Display ID is required")
        if AIContest.objects.exclude(id=problem_id).filter(_id=_id, contest=contest).exists():
            return self.error("Display ID already exists")
    ...
    def delete(self, request):
        ...
        if not id:
            return self.error("Invalid parameter, id is required")
        try:
            problem = AIContest.objects.get(id=id, contest_id__isnull=False)
        except AIContest.DoesNotExist:
            return self.error("Problem does not exists")
        ensure_created_by(problem.contest, request.user)
...
class MakeContestProblemPublicAPIView(APIView):
    ...
    def post(self, request):
        ...
        if AIContest.objects.filter(_id=display_id, contest_id__isnull=True).exists():
            return self.error("Duplicate display ID")

        try:
            problem = AIContest.objects.get(id=data["id"])
        except AIContest.DoesNotExist:
            return self.error("Problem does not exist")
...
class AddContestProblemAPI(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            problem = AIContest.objects.get(id=data["problem_id"])
        except (Contest.DoesNotExist, AIContest.DoesNotExist):
            return self.error("Contest or Problem does not exist")

        if contest.status == ContestStatus.CONTEST_ENDED:
            return self.error("Contest has ended")
        if AIContest.objects.filter(contest=contest, _id=data["display_id"]).exists():
...
class ExportProblemAPI(APIView):
    ...
    def get(self, request):
        problems = AIContest.objects.filter(id__in=request.data["problem_id"])
            ...
            with transaction.atomic():
                for i in range(1, count + 1):
                    with zip_file.open(f"{i}/problem.json") as f:
                        ...
                        problem_obj = AIContest.objects.create(_id=problem_info["display_id"],
                                                             ...
                                                             end_time=problem_info["endf_time"]["value"],
                        ...
                        for tag_name in problem_info["tags"]:
                            tag_obj, _ = AIContestTag.objects.get_or_create(name=tag_name)
...
class FPSProblemImport(CSRFExemptAPIView):
    ...
    def _create_problem(self, problem_data, creator):
        ...
        AIContest.objects.create(_id=f"fps-{rand_str(4)}",
